...
public class DiamondCoinBundleItem extends BundleItem {
    private static final int WITHDRAW_AMOUNT = 1000;

    public DiamondCoinBundleItem(Properties properties) {
        super(properties);
    }
    ...
    protected InteractionResultHolder<ItemStack> depositCoins(Player player, ItemStack bundle) {
        ...
        int coinsInInventory = getCoinsInInventory(player, CoinType.DIAMOND);
        ...
        removeCoinsFromInventory(player, toDeposit, CoinType.DIAMOND);
    ...
    protected InteractionResultHolder<ItemStack> withdrawCoins(Player player, ItemStack bundle) {
        ...
        // 优先使用大面额钻石币
        Item fiftyCoin = ItemsRegistry.FIFTY_DIAMOND_COINS.get();
        ...
        if (remaining >= 10) {
            Item tenCoin = ItemsRegistry.TEN_DIAMOND_COINS.get();
        ...
        if (remaining >= 5) {
            Item fiveCoin = ItemsRegistry.FIVE_DIAMOND_COINS.get();
        ...
        if (remaining > 0) {
            Item oneCoin = ItemsRegistry.DIAMOND_COIN.get();
